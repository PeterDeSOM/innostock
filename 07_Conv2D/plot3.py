...
def parse_dataset(df_source, window_size):
	...
	for i in range(len(df_target) - window_size):
		dataset_X.append([convert_to_image(df_target.iloc[i:(i + window_size), :-1])])
		dataset_Y.append([df_target.loc[i + window_size - 1, 'target_value']])
...
def convert_to_image(df):
	...
	for volume in df['volume'].values:
		vol_colors.append('#000000' if volume >= old_volume else '#999999')
		old_volume = volume
	...
	for open, high, offer, bid, low, current, dod, volume, eopen, ehigh, elow, eclose in \
		...
		vols.append(volume)
		dods.append(dod)
		dod_colors.append('#000000' if dod >= 0 else '#999999')
	...
	ax0.plot(date, df['rsi'], '#333333', linewidth=1)
	...
	candlestick_ohlc(ax1, e_dt, width=.4, colorup='#000000', colordown='#999999')

	ax2 = plt.subplot2grid((8, 4), (3, 0), sharex=ax0, rowspan=3, colspan=2)
	ax2.set_frame_on(True)
	ax2.get_yaxis().set_visible(False)
	ax2.get_xaxis().set_visible(False)
	candlestick_ohlc(ax2, data, width=.4, colorup='#000000', colordown='#999999')
	plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))

	ax3 = plt.subplot2grid((8, 4), (6, 0), sharex=ax0, rowspan=2, colspan=2)
	ax3.bar(date, df['ma02'], width=.4, align='center', color='#000000')
	ax3.bar(date, df['ma03'], width=.4, align='center', color='#999999')
	...
	""" ### RIGHT """
	nema = 9
	ax5 = plt.subplot2grid((8, 4), (0, 2), sharex=ax1, rowspan=1, colspan=2)
	emaslow, emafast, macd = df['emaslow'], df['emafast'], df['macd']
	ema9 = ExpMovingAverage(macd, nema)
	ax5.plot(date, macd, color='#555555', lw=1)
	ax5.plot(date, ema9, color='#999999', lw=1)
	ax5.fill_between(date, macd - ema9, 0, facecolor='#bbbbbb')
	ax5.set_frame_on(True)
	ax5.get_yaxis().set_visible(False)
	ax5.get_xaxis().set_visible(False)

	ax8 = plt.subplot2grid((8, 4), (1, 2), sharex=ax0, rowspan=2, colspan=2)
	ax8.bar(date, dods, width=.4, align='center', color=dod_colors)
	ax8.set_frame_on(True)
	ax8.get_yaxis().set_visible(False)
	ax8.get_xaxis().set_visible(False)

	ax4 = plt.subplot2grid((8, 4), (3, 2), sharex=ax0, rowspan=3, colspan=2)
	ax4.bar(date, vols, width=.4, align='center', color=df['vol_colors'])
	ax4.set_frame_on(True)
	ax4.get_yaxis().set_visible(False)
	ax4.get_xaxis().set_visible(False)

	ax9 = plt.subplot2grid((8, 4), (6, 2), sharex=ax0, rowspan=2, colspan=2)
	ax9.bar(date, df['ma05'], width=.4, align='center', color='#000000')
	ax9.bar(date, df['ma10'], width=.4, align='center', color='#999999')
