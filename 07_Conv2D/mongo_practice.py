...
class Source(object):
	def __init__(self):
		self._MONGOCONN_ = MongoClient('mongodb://%s:%s@192.168.233.134:27017' % (parse.quote_plus('superclient'), parse.quote_plus('Infor3233!@#')))
		self._DB_ = self._MONGOCONN_.innostock
		self._SYMBOLS_SOURCE_ = self._DB_.ConvertedPlotImages.aggregate([
			{"$group": {"_id": "$isin", "count": {"$sum": 1}}}
		])

		self._SYMBOLS_SOURCE_ = pd.DataFrame(list(self._SYMBOLS_SOURCE_))
		self._SYMBOLS_SOURCE_.columns = ['isin', 'count']
		...
		self._SYMBOLS_SOURCE_ = pd.merge(self._SYMBOLS_SOURCE_, df, on='isin')
	...
	def next(self):
		...
		self._SYMBOL_DATASET_ = pd.DataFrame(list(self._DB_.ConvertedPlotImages.find({'isin':'KR7001040005', 'trans_date':'2017-11-01'})))
		print(self._SYMBOL_DATASET_['plot_image'])
		exit()


		query_string = 'SELECT * FROM drl_1d WHERE isin = %s ORDER BY trans_date'
		values = [isin]
		self._SYMBOL_DATASET_ = self._DB_.select(query_string, values)
