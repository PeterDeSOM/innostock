...
_MODEL_WEIGHT_DIR_ = 'graduated/a3c/'
...
class A3CAgent:
    ...
    def build_model(self):
        input_state = Input(shape=(self.input_height, self.input_width, self.input_chanel))

        # ===========ENTRY FLOW==============
        # Block 01 ---
        x = Conv2D(32, (3, 3), strides=(2, 2))(input_state)
...
class Agent(threading.Thread):
    def __init__(self, creator):
        ...
        self.input_height = creator.input_height
        self.input_width = creator.input_width
        self.input_chanel = creator.input_chanel
    ...
    def run(self):
        ...
        while epoch_current <= epoch and not stop_running:
            while failure_current <= failure_limit and steps_current <= steps_an_episode and not stop_running:
                time.sleep(np.round(random.uniform(.0, .1), 3))

                state, y_value = next(train_generator)
                y_value = np.argmax(y_value)
                rate_applied = train_generator.total_batches_seen / train_generator.n
                ...
                if train_generator.total_batches_seen == train_generator.n:
    ...
    def discounted_prediction(self):
        ...
        running_add = self.critic.predict(self.states[-1].reshape((1, self.input_height, self.input_width, self.input_chanel)))
