...
def get_investor_transactions(inv_code, transdate):
    ...
    df.columns = ['symb_code',
                  ...
                  'amt_selling',
                  'dummy2']
    df = df.drop(['dummy0', 'dummy1', 'dummy2'], axis=1).fillna('')
...
querystring = 'SELECT MAX(trans_date) trans_date FROM krx_trans_d_investors'
...
for i in range(daysbefore, -1, -1):
    ...
    i = 1
    for inv_code, inv_name in df_investors.values:
        if proc_stop:
            break

        printProgress(i, len(df_investors), '# Progress :', '(%s) %s\'s data is processing...' % (
            transdate.strftime('%Y-%m-%d'), inv_name), 2, 50)
        i += 1
