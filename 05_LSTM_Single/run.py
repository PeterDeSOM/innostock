...
from lstm.env import Activities
from lstm.dataset import Source
...
class A3CAgent:
    def __init__(self, name, dataset, input_dim, input_length, output_dim):
    ...
    def build_model(self):
        ...
        layer1 = GRU(self.state_size * self.input_length, activation='relu', return_sequences=True)(input_state)
        layer2 = GRU(self.state_size * self.input_length, activation='relu', return_sequences=True)(layer1)
        layer3 = GRU(self.state_size * self.input_length, activation='relu', return_sequences=True)(layer2)
        layer4 = GRU(self.state_size, activation='relu')(layer3)

        policy = Dense(self.action_size, activation='softmax')(layer4)
        value = Dense(1, activation='linear')(layer4)
...
class Agent(threading.Thread):
    def __init__(self, creator, datasource_name, datasource, state_size, input_length, action_size, model, sess, optimizer,
                 discount_factor):
    ...
    def run(self):
        ...
        env_act = Activities(self._DATASOURCE_, self.state_size, self.input_length, self.action_size)
...
def main():
    ...
    for i in range(source_size):
        ...
        dataset_info[1], dataset_info[2], input_dim, i + 1, source_size))

        agent = A3CAgent(name, dataset, input_dim, input_length, output_dim)
        agent.train()

    print(
        '##### All the trainings are finished. -----------------------------------------------------------------------')
