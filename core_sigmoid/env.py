import numpy as np
from core_sigmoid.dataset import Controller
...
class Activities(object):
	...
	def _parse_current_observation(self):
		state = np.nan_to_num(self._STATE_[0:-1].astype('float'))
	...
	def step(self, action_probability, y_value):
		done = ~(action_probability.item() >= y_value-.05 and action_probability.item() <= y_value+.05)
