...
from core_sigmoid.env import Activities
from core_sigmoid.dataset import Source
...
class A3CAgent:
	...
	def build_model(self):
		...
		policy = Dense(self.action_size, activation='sigmoid')(layer6)
	...
	def save(self):
		...
		if not os.path.exists(_MODEL_WEIGHT_DIR_):
			os.makedirs(_MODEL_WEIGHT_DIR_)

		model_actor_located = '%sV%s_MODEL_W_ACTOR_.h5' % (_MODEL_WEIGHT_DIR_, mean_acc)
		model_critic_located = '%sV%s_MODEL_W_CRITIC_.h5' % (_MODEL_WEIGHT_DIR_, mean_acc)
...
class Agent(threading.Thread):
	...
	def get_action(self, state):
		...
		if np.random.random() < self._EPSILON_:
			action_probability = np.random.uniform(0., 1., self.action_size).reshape((1, self.action_size))
			action_type = 0
		else:
			action_probability = self.local_actor.predict(state)
			self.STACK_HISTORY_HOLDER_MODEL_ACTION_RANGE.append(action_probability)
	...
	def get_progressing(self):
		mean_actor = 0 if len(self.STACK_HISTORY_HOLDER_ACTOR_LOSS) == 0 else np.nan_to_num(np.mean(self.STACK_HISTORY_HOLDER_ACTOR_LOSS))
		mean_critic = 0 if len(self.STACK_HISTORY_HOLDER_CRITIC_LOSS) == 0 else np.nan_to_num(np.mean(self.STACK_HISTORY_HOLDER_CRITIC_LOSS))
		mean_model_predict = np.mean(self.STACK_HISTORY_HOLDER_MODEL_PREDICT)
		action_range = np.round(np.mean(np.vstack(self.STACK_HISTORY_HOLDER_MODEL_ACTION_RANGE), axis=0), 2) \
			if len(self.STACK_HISTORY_HOLDER_MODEL_ACTION_RANGE) > 0 else np.zeros(7)
		...
		return {
			'LOSS': '%s%s %s%s %s' % (
				...
				'{0:.2f}'.format(np.mean(self.STACK_HISTORY_HOLDER_ACCURACY))
			),
			'PREDICT': '%s %s' % (
				'{0:.2f}'.format(mean_model_predict),
				action_range
			),
			'PROGRESS': self.CACHE_PROGRESS_CURRENT
		}
	...
	def train_model(self):
		...
		self.states -= np.mean(self.states, axis=0)
		self.rewards -= np.mean(self.rewards, axis=0)
...
def main():
	...
	for i in range(source_size):
		if len(datasource.next()) == 0:
			break

		name, dataset, input_dim, output_dim = datasource.get_dateset()
		print('%s started (%s of %s)' % (name, i+1, source_size))
