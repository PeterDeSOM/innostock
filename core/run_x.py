...
class Activities(object):
	...
	def step(self, action_probability, action_type):
		...
		# reward = (action - y_value)
		# reward = (reward / reward**2) if reward != 0 else 0
...
class Keractoritic(object):
	...
	def __init__(self, sess, out_dim, in_dim, training_size):
		...
		self._DIM_INPUT_ = in_dim
		self._DIM_OUTPUT_ = out_dim
		self._RATE_LEARNING_ACTOR_ = 0.001
		self._RATE_LEARNING_CRITIC_ = 0.001
		self._EPSILON_ = 1.
		self._EPSILON_DECAY_ = .9995
		self._EPSILON_RANDOM_MIN_ = .05
		self._EPSILON_RANDOM_MAX_ = .5
		self._GAMMA_DISCOUNT_FACTOR_ = .85
		self._TAU_ = .095
		self._COUNT_TOTAL_RUNNING_ = 0.
		self._COST_ACTOR_ = .0
		...
		self._ACTOR_TRAINING_SIZE_ = training_size
		self._ACTOR_TRAINING_STARTED_ = False
		self._STACK_ACTION_RESULT_ACCURACY_HOLDER_SIZE = training_size
		self._STACK_ACTION_RESULT_ACCURACY_HOLDER_ = deque(maxlen=self._STACK_ACTION_RESULT_ACCURACY_HOLDER_SIZE * 2)
		...
		# dC/dA (from actor)
		self._GRADIENT_ACTOR_ = tf.gradients(self._MODEL_ACTOR_.output, trainable_weights_actor, -self._GRADIENT_ACTOR_CRITIC_)

		gradients_actor = zip(self._GRADIENT_ACTOR_, trainable_weights_actor)
		self._COST_OPTIMIZED_ = tf.train.AdamOptimizer(self._RATE_LEARNING_ACTOR_).apply_gradients(gradients_actor)
		...
		self._GRADIENT_CRITIC_ = tf.gradients(self._MODEL_CRITIC_.output, self._ACTION_INPUT_CRITIC_)
	...
	def _actor(self):
		...
		layer1 = Dense(int(self._DIM_INPUT_**2 / 2), activation='relu')(input_state)
		layer2 = Dense(int(self._DIM_INPUT_**2 / 2), activation='relu')(layer1)
		layer3 = Dense(int(self._DIM_INPUT_**2 / 2), activation='relu')(layer2)
		output = Dense(self._DIM_OUTPUT_, activation='softmax')(layer3)

		model = Model(inputs=input_state, outputs=output)
		model.compile(loss='mse', optimizer=Adam(lr=self._RATE_LEARNING_ACTOR_), metrics=['accuracy'])
	...
	def _critic(self):
		input_state = Input(shape=(self._DIM_INPUT_,))
		layer1 = Dense(int(self._DIM_INPUT_**2 / 2), activation='relu')(input_state)
		layer2 = Dense(int(self._DIM_INPUT_**2 / 2))(layer1)

		input_action = Input(shape=(self._DIM_OUTPUT_,))
		layer_action = Dense(int(self._DIM_INPUT_**2 / 2))(input_action)

		merged = Add()([layer2, layer_action])
		layer_merged = Dense(int(self._DIM_INPUT_**2 / 2), activation='relu')(merged)

		output = Dense(self._DIM_OUTPUT_, activation='relu')(layer_merged)

		model = Model(inputs=[input_state, input_action], outputs=output)
		model.compile(loss='mse', optimizer=Adam(lr=self._RATE_LEARNING_CRITIC_), metrics=['accuracy'])
	...
	def train(self):
		if len(self._STACK_HISTORY_HOLDER_) < self._ACTOR_TRAINING_SIZE_:
			return
	...
	def _train_critic(self):
		...
		arr_action_stochastic_policy, arr_real_action_stochastic_policy = [], []
		arr_history = []

		samples = random.sample(self._STACK_HISTORY_HOLDER_, self._ACTOR_TRAINING_SIZE_)
		...
		for sample in samples:
			...
			future_reward = self._MODEL_CRITIC_TARGET_.predict([state_new, target_action])
			reward += self._GAMMA_DISCOUNT_FACTOR_ * future_reward
			# reward += self._GAMMA_DISCOUNT_FACTOR_ * real_action_stochastic_policy

			hist = self._MODEL_CRITIC_.fit([state_current, action_probability], reward, verbose=0)
			arr_history.append(hist.history['loss'][0])

			arr_state.append(state_current)
			arr_reward.append(reward)
			arr_action_stochastic_policy.append(action_probability)
			arr_real_action_stochastic_policy.append(real_action_stochastic_policy)

			accuracy_.append(int(~done))

		arr_state = np.vstack(arr_state)
		arr_reward = np.vstack(arr_reward)
		arr_action_stochastic_policy = np.vstack(arr_action_stochastic_policy)
		arr_real_action_stochastic_policy = np.vstack(arr_real_action_stochastic_policy)
		...
		"""
		hist = self._MODEL_CRITIC_.fit(
			[arr_state, arr_action_stochastic_policy],
			arr_reward,
			verbose=0,
			batch_size=self._ACTOR_TRAINING_SIZE_
		)
		"""
		self._COST_CIRITIC_	= self._MODEL_ACTOR_.evaluate(
			arr_state,
			arr_real_action_stochastic_policy,
			verbose=0
		)
		# self._COST_CIRITIC_.append(hist.history['loss'][0])
		self._COST_CIRITIC_.append(np.mean(arr_history))
		self._STACK_ACTION_RESULT_ACCURACY_HOLDER_.append(self._COST_CIRITIC_[1])

		self._ACCURACY_CIRITIC_ = np.sum(accuracy_) / self._STACK_ACTION_RESULT_ACCURACY_HOLDER_SIZE


	def _train_actor(self):
		arr_state, arr_real_action_stochastic_policy, arr_reward, arr_action_label, arr_posicygradient_errors = [], [], [], [], []
		arr_history = []

		samples = random.sample(self._STACK_HISTORY_HOLDER_, self._ACTOR_TRAINING_SIZE_)

		for sample in samples:
			state_current, action_probability, reward, _, _, real_action_stochastic_policy = sample
			...
			# cost = tf.reduce_mean(-tf.reduce_sum(real_action_stochastic_policy * tf.log(self._MODEL_ACTOR_.outputs[0]), axis=1))
			# cost = self._SESSION_.run(cost, feed_dict={self._STATE_INPUT_ACTOR_: state_current})
			# arr_history.append(cost)
		...
		cost = tf.reduce_mean(-tf.reduce_sum(arr_real_action_stochastic_policy * tf.log(self._MODEL_ACTOR_.outputs[0]), axis=1))
		cost = self._SESSION_.run(cost, feed_dict={self._STATE_INPUT_ACTOR_: arr_state})
		"""
		...
		# self._COST_ACTOR_.append(cost)
		# self._COST_ACTOR_.append(np.mean(arr_history))
	...
	def learning_info(self):
		return self._COST_CIRITIC_, \
		       self._COST_ACTOR_, \
		       self._ACCURACY_CIRITIC_, \
		       np.mean(self._STACK_ACTION_RESULT_ACCURACY_HOLDER_) if np.sum(self._STACK_ACTION_RESULT_ACCURACY_HOLDER_) > 0 else .0
...
def main():
	...
	env_kac = Keractoritic(sess, out_dim, in_dim, training_size)
	...
	while epoch_current < epoch:
		cost1, cost2, accuracy, accuracy_mean_actor = env_kac.learning_info()
		print('(%s / %s / %s) EPOCH: %s, EPISOD: %s # COST: %s (%s) / %s, ACCURACY: %s / %s / %s / %s %s' % (
						'{0:4,}'.format(symb_idx),
						...
						'{0:.7f}'.format(cost1[2] if type(cost1) is list else 0.),
						...
						'{0:.2f}'.format(accuracy_mean_actor),
						sending_massage
					)
		      )
